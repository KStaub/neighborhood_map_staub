...
function ViewModel() {
	...
	this.infoWindowContent = function(marker) {
		var html = '<div data-bind="with: currentMarker">' +
	...
	this.infoWindowEditMode = function(marker) {
		var html = '<div>' +
			'<form id="editForm">' +
				...
				'<button type="button" id="confirmEditButton" data-bind="click: function() { $root.submitEdit }"></button>' +
				'<button type="button" id="liDeleteButton" data-bind="click: function() { $root.deleteListItem(marker) }"></button>' +
			'</form>' +
		'</div>';
